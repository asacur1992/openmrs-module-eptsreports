...
public abstract class TxMLPatientCalculation extends BaseFghCalculation {
	...
	public CalculationResultMap evaluate(
		...
		CalculationResultMap lastFilaCalculationResult = Context.getRegisteredComponents(LastFilaCalculation.class)
		        .get(0)
		        .evaluate(cohort, parameterValues, context);
		CalculationResultMap lastSeguimentoCalculationResult = Context
		        .getRegisteredComponents(LastSeguimentoCalculation.class)
		        .get(0)
		        .evaluate(cohort, parameterValues, context);
		...
		CalculationResultMap nextFilaResult = Context.getRegisteredComponents(NextFilaDateCalculation.class)
		        .get(0)
	...
	protected CalculationResultMap filterUntracedAndTracedPatients(
		...
		Map<Integer, Date> criteriaOneData = queryDisaggregation
		        .findUntracedPatientsWithinReportingPeriodCriteriaOne(context);
		ListMap<Integer, Date> criteriaTwoData = queryDisaggregation
		        .findUntracedByNotHavefilledDataInVisitSectionCriteriaTwo(context);
		ListMap<Integer, Date> criteriaThreeData = queryDisaggregation
		        .findTracedPatientsWithinReportingPeriodCriteriaThree(context);
		ListMap<Integer, Date> criteriaThreeNegationData = queryDisaggregation
		        .findTracedPatientsWithinReportingPeriodCriteriaThreeNegation(context);
