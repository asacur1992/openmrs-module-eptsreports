  ...
  class QUERY {
    ...
    public static final String findPatietsWhoAreOnAdherenceClubs =
        "SELECT max_ca.patient_id FROM 	( "
            ...
            + "INNER JOIN encounter e on p.patient_id=e.patient_id "
            + "INNER JOIN obs o on o.encounter_id=e.encounter_id "
            + "WHERE e.encounter_type=6 AND e.voided=0 AND o.voided=0 AND p.voided=0 AND o.concept_id=23726 and e.location_id=:location AND o.obs_datetime<=:endDate "
            + "GROUP BY p.patient_id "
            + ") max_ca "
            + "INNER JOIN encounter e on e.patient_id=max_ca.patient_id "
            + "INNER JOIN obs on obs.encounter_id=e.encounter_id "
            + "WHERE max_ca.max_dataca=obs.obs_datetime and obs.concept_id=23726 AND obs.value_coded IN (1256,1257) AND obs.location_id=:location and obs.voided=0 and e.encounter_type=6 and e.voided=0 ";
    ...
    public static final String findPatientsWhoAreFastTrack =
        "select patient_id from( "
            ...
            + "inner join encounter e on p.patient_id=e.patient_id "
            + "where p.voided=0 and e.voided=0 and e.encounter_type in (6,9) and  date(e.encounter_datetime) <= :endDate and e.location_id=:location "
            + "group by p.patient_id "
            + ") ultimaConsulta "
    ...
    public static final String findPatientsWhoAreFamilyAproach =
        "select max_af.patient_id from ("
            ...
            + "inner join obs o on o.encounter_id=e.encounter_id "
            + "where e.encounter_type=6 and e.voided=0 and o.voided=0 and p.voided=0 "
            + "and o.concept_id=23725 and e.location_id=:location and o.obs_datetime<=:endDate group by p.patient_id "
            + ") max_af "
            + " inner join encounter e on e.patient_id=max_af.patient_id "
            + "inner join obs on obs.encounter_id=e.encounter_id "
            + "where max_af.max_dataaf=obs.obs_datetime and obs.concept_id=23725 and obs.value_coded in (1256,1257) and obs.voided=0 and obs.location_id=:location and e.encounter_type=6 and e.voided=0 ";
