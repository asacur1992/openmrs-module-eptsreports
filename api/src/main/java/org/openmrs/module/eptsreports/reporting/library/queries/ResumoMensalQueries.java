...
public class ResumoMensalQueries {
  ...
  public static String getAllPatientsWithPreArtStartDateLessThanReportingStartDateA1() {
    final String query = EptsQuerysUtils.loadQuery(ResumoMensalQueries.FIND_PRE_TARV_PATIENT_A1);
    return query;
  }
  ...
  public static String getAllPatientsWithPreArtStartDateWithBoundariesA2() {
    final String query = EptsQuerysUtils.loadQuery(ResumoMensalQueries.FIND_PRE_TARV_PATIENT_A2);
    return query;
  }

  public static String getPatientsWhoMarkedINHC2() {
    final String query = EptsQuerysUtils.loadQuery(ResumoMensalQueries.FIND_PRE_TARV_INH_3HP_C2);
    return query;
  }
  ...
  public static String findPatientsWhoAreCurrentlyEnrolledOnArtMOHLastMonthB12() {
    ...
    final String query =
        "				select patient_id "
            ...
            + "                                         and o.value_datetime is not null and  o.value_datetime<:startDate and e.location_id=:location "
            + "                                         group by p.patient_id "
            + "                                     ) "
            ...
            + "                                                 inner join patient_program pg on p.patient_id=pg.patient_id "
            + "                                                 inner join patient_state ps on pg.patient_program_id=ps.patient_program_id "
            + "                                             where pg.voided=0 and ps.voided=0 and p.voided=0 and pg.program_id=2 and ps.start_date<:startDate and pg.location_id=:location "
            + "                                                 group by p.patient_id "
            + "                                             ) "
            + "                                         maxEstado "
            + "                                             inner join patient_program pg2 on pg2.patient_id=maxEstado.patient_id "
            ...
            + "                                             inner join obs  o on e.encounter_id=o.encounter_id "
            + "                                         where e.voided=0 and o.voided=0 and p.voided=0 and  e.encounter_type in (53,6) and o.concept_id in (6272,6273) "
            + "                                             and o.value_coded in (1706,1366) and o.obs_datetime<:startDate and e.location_id=:location "
            + "                                             group by p.patient_id "
            + "                                         union "
            ...
            + "                                     from patient p "
            + "                                         inner join encounter e on e.patient_id=p.patient_id "
            + "                                     where p.voided=0 and e.voided=0 and e.encounter_type in (6,9) and  e.location_id=:location and e.encounter_datetime<:startDate "
            + "                                         group by p.patient_id "
            + "                                 ) "
            ...
            + "                     group by patient_id "
            + "                ) coorte12meses_final "
            + "            where ((data_estado is null or ((data_estado is not null and decisao=2) and  data_usar_c>data_estado))  and date_add(data_usar, interval 60 day) >=(:startDate - interval 1 day) ) "
