...
@Component
public class TxCurrCommunityBMDataset extends BaseDataSet {
  ...
  public CohortIndicatorDataSetDefinition constructTxCurrDataset(final boolean currentSpec) {
    ...
    final CohortDefinition txCurrCompositionCohort =
        this.txCurrCohortQueries.findCommunityBMPatientsWhoAreActiveOnART();
    ...
    dataSetDefinition.addColumn(
        "CC1All", "TX_CURR: Currently on ART", EptsReportUtils.map(txCurrIndicator, mappings), "");
  ...
  private String getName(final Gender gender, final AgeRange ageRange) {
    String name = "CC-males-" + ageRange.getName() + "" + gender.getName();

    if (gender.equals(Gender.FEMALE)) {
      name = "CC-females-" + ageRange.getName() + "" + gender.getName();
    }
  ...
  private void setPatientsOnrArvLess3MonthsColumnsDisagregations(
    ...
    for (final ColumnParameters column : this.getColumnsForArvDispenseDisagregatioins()) {
      final String name = "CC1" + "-" + column.getColumn();
      final String label =
  ...
  private void setPatientsOnrArvFor3and5MonthsColumnsDisagregations(
    ...
    for (final ColumnParameters column : this.getColumnsForArvDispenseDisagregatioins()) {
      final String name = "CC2" + "-" + column.getColumn();
      final String label =
  ...
  private void setPatientsOnrArvFor6OrMoreMonthsColumnsDisagregations(
    ...
    for (final ColumnParameters column : this.getColumnsForArvDispenseDisagregatioins()) {
      final String name = "CC3" + "-" + column.getColumn();
      final String label =
