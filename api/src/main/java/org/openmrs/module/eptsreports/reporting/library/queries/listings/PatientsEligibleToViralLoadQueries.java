  ...
  class QUERY {
    public static final String findPatientsEligibleToViralLoad =
        "SELECT p.patient_id, pi.identifier, CONCAT(pn.given_name, ' ', COALESCE(pn.middle_name, ''), ' ', pn.family_name)name, pe.gender, (YEAR( :endDate) - YEAR(pe.birthdate)) age,\n"
            ...
            + "AND ((ABS(DATEDIFF(art_start_date, :endDate)) >= 180 AND (first_viral_load_date IS NULL OR (ABS(DATEDIFF(last_viral_load_date, :endDate)) >= 365 AND last_viral_load_value < 1000 ))) \n"
            + "    OR (ABS(DATEDIFF(last_regimen_change_date, :endDate)) >= 180 AND (last_viral_load_date_regimen IS NUll OR (ABS(DATEDIFF(last_viral_load_date_regimen, :endDate)) >= 365 AND last_viral_load_value_regimen < 1000))) \n"
            + "    OR (ABS(DATEDIFF(restart_art_date, :endDate)) >= 180 AND abandonment_date IS NOT NULL AND (viral_load_restart_art IS NULL OR (ABS(DATEDIFF(last_viral_load_date_restart_art, :endDate)) >= 365 AND last_viral_load_value_restart_art < 1000 ))))\n"
