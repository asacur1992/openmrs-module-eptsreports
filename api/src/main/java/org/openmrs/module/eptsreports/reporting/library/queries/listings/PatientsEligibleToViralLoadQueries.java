  ...
  class QUERY {
    public static final String findPatientsEligibleToViralLoad =
        "SELECT p.patient_id, pi.identifier, CONCAT(pn.given_name, ' ', COALESCE(pn.middle_name, ''), ' ', pn.family_name)name, pe.gender, (YEAR( :endDate) - YEAR(pe.birthdate)) age,\n"
            ...
            + "    LEFT JOIN\n"
            + "    (\n"
            + "        SELECT last_viral_load_date_regimen.patient_id,last_viral_load_date_regimen, last_viral_load_value_regimen, last_viral_load_value_regimen_coded FROM\n"
            + "        (\n"
            ...
            + "        LEFT JOIN\n"
            + "        (\n"
            + "            SELECT o.person_id patient_id, o.obs_datetime ,o.value_numeric last_viral_load_value_regimen, o.value_coded last_viral_load_value_regimen_coded FROM obs o\n"
            + "	            WHERE o.voided = 0 AND o.concept_id IN (856,1305)\n"
            ...
            + "    LEFT JOIN\n"
            + "    (\n"
            + "        SELECT last_viral_load_date_restart_art.patient_id,last_viral_load_date_restart_art, last_viral_load_value_restart_art, last_viral_load_value_restart_art_coded FROM\n"
            + "        (\n"
            ...
            + "        LEFT JOIN\n"
            + "        (\n"
            + "            SELECT o.person_id patient_id, o.obs_datetime ,o.value_numeric last_viral_load_value_restart_art, o.value_coded last_viral_load_value_restart_art_coded FROM obs o\n"
            + "	            WHERE o.voided = 0 AND o.concept_id IN (856,1305)\n"
            ...
            + "AND ((DATEDIFF( :endDate, art_start_date) >= 180 AND last_regimen_change_date IS NULL AND restart_art_date IS NULL AND (last_viral_load_date IS NULL OR DATEDIFF( :endDate, last_viral_load_date) >= 365 AND (last_viral_load_value < 1000 OR last_viral_load_value_coded NOT IN (1306,23907,23905,23904,23906,23814,23908,1087))))\n"
            + "    OR (DATEDIFF( :endDate, last_regimen_change_date) >= 180 AND (last_viral_load_value_regimen IS NUll OR (DATEDIFF( :endDate, last_viral_load_date_regimen) >= 365 AND (last_viral_load_value_regimen < 1000 OR last_viral_load_value_regimen_coded NOT IN (1306,23907,23905,23904,23906,23814,23908,1087)))))\n"
            + "    OR (DATEDIFF( :endDate, restart_art_date) >= 180 AND (abandonment_date IS NOT NULL AND (last_viral_load_date_restart_art IS NULL OR DATEDIFF( :endDate, last_viral_load_date_restart_art) >= 365 AND (last_viral_load_value_restart_art < 1000 OR last_viral_load_value_restart_art_coded NOT IN (1306,23907,23905,23904,23906,23814,23908,1087) )))))  \n"
