  ...
  class QUERY {
    public static final String findPatientsEligibleToViralLoad =
        "SELECT p.patient_id, pi.identifier, CONCAT(pn.given_name, ' ', COALESCE(pn.middle_name, ''), ' ', pn.family_name)name, pe.gender, (YEAR( :endDate) - YEAR(pe.birthdate)) age,\n"
            ...
            + "AND ((DATEDIFF( :endDate, art_start_date) >= 180 AND last_regimen_change_date IS NULL AND restart_art_date IS NULL AND (last_viral_load_date IS NULL OR DATEDIFF( :endDate, last_viral_load_date) >= 365 AND last_viral_load_value < 1000 ))\n"
            + "    OR (DATEDIFF( :endDate, last_regimen_change_date) >= 180 AND (last_viral_load_value_regimen IS NUll OR (DATEDIFF( :endDate, last_viral_load_date_regimen) >= 365 AND last_viral_load_value_regimen < 1000)))\n"
            + "    OR (DATEDIFF( :endDate, restart_art_date) >= 180 AND (abandonment_date IS NOT NULL AND (last_viral_load_date_restart_art IS NULL OR DATEDIFF( :endDate, last_viral_load_date_restart_art) >= 365 AND last_viral_load_value_restart_art < 1000 ))))  \n"
