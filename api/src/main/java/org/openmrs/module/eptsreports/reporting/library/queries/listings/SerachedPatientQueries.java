...
public interface SerachedPatientQueries {
	...
	class QUERY {
		public static final String findSearchedPatientsList =
				"SELECT p.patient_id, pi.identifier, CONCAT(pn.given_name, ' ', COALESCE(pn.middle_name, ''), ' ', pn.family_name)name, pe.gender, (YEAR( :endDate) - YEAR(pe.birthdate)) age, patient_contact, CONCAT(pa.address1,',',pa.address3,',',pa.address5,',',pa.address6) address, sector, type_of_visit, start_art.art_start_date, expected_date, GREATEST(DATEDIFF( :endDate, expected_date), 0) missing_days, patients_found.last_visit, last_encounter.encounter_date, volunteer FROM patient p\n"
						...
						+ "						AND o.concept_id IN (2016, 2157, 2158)\n"
						...
						+ "LEFT JOIN\n"
						+ "(\n"
						+ "	SELECT p.patient_id, MAX(e.encounter_datetime) encounter_date FROM patient p\n"
						+ "		INNER JOIN encounter e ON e.patient_id = p.patient_id\n"
						+ "			WHERE e.voided = 0 AND p.voided = 0 AND e.encounter_type IN (6,18,35,53)\n"
						+ "					GROUP BY p.patient_id\n"
						+ ")last_encounter ON patients_found.patient_id = last_encounter.patient_id AND encounter_date >= last_visit\n"
						+ "\n"
						+ "LEFT JOIN \n"
						+ "(\n"
						...
						+ "LEFT JOIN\n"
						+ "(\n"
						+ "	SELECT last_visit.patient_id, CASE \n"
						+ "		WHEN last_sector.value_coded = 2175 THEN 'TARV (Clínica/APSS e PP)'\n"
						+ "		WHEN last_sector.value_coded = 1872 THEN 'CCR'\n"
						+ "		WHEN last_sector.value_coded = 1598 THEN 'CPN'\n"
						+ "		WHEN last_sector.value_coded = 1414 THEN 'PNCT'\n"
						+ "		WHEN last_sector.value_coded = 1987 THEN 'SAAJ'\n"
						+ "		WHEN last_sector.value_coded = 23913 THEN 'OUTRO'\n"
						+ "		ELSE null END AS sector, last_sector.encounter_datetime FROM (\n"
						+ "		SELECT p.patient_id, MAX(e.encounter_datetime) visit_date FROM patient p\n"
						+ "			INNER JOIN encounter e ON e.patient_id = p.patient_id\n"
						+ "			INNER JOIN obs o ON o.encounter_id = e.encounter_id\n"
						+ "				WHERE p.voided = 0 AND e.voided = 0 AND o.voided = 0 AND e.encounter_type = 21 \n"
						+ "					AND o.concept_id = 2176\n"
						+ "					AND e.location_id = :location AND e.encounter_datetime BETWEEN ( :startDate - INTERVAL 15 DAY) AND ( :endDate - INTERVAL 15 DAY)\n"
						+ "						GROUP BY p.patient_id\n"
						+ "	)last_visit\n"
						+ "	INNER JOIN (\n"
						+ "		SELECT o.person_id patient_id, o.value_coded, e.encounter_datetime FROM obs o\n"
						+ "			INNER JOIN encounter e ON e.encounter_id = o.encounter_id\n"
						+ "				WHERE e.voided = 0 AND o.voided = 0 AND e.encounter_type = 21\n"
						+ "				AND e.location_id = :location AND e.encounter_datetime BETWEEN ( :startDate - INTERVAL 15 DAY) AND ( :endDate - INTERVAL 15 DAY)\n"
						+ "				AND o.concept_id = 2176\n"
						+ "					GROUP BY patient_id\n"
						+ "	)last_sector ON last_visit.patient_id = last_sector.patient_id AND visit_date = encounter_datetime\n"
						+ ")sector ON p.patient_id = sector.patient_id\n"
						+ "\n"
						+ "LEFT JOIN \n"
						+ "(\n"
						+ "	SELECT last_visit.patient_id, CASE \n"
						+ "		WHEN last_reason.value_coded = 2161 THEN 'Visita Preventiva'\n"
						+ "		WHEN last_reason.value_coded = 2160 THEN 'Visita de Reintegração'\n"
						+ "		WHEN last_reason.value_coded = 23914 THEN 'Visita aos Casos Especiais'\n"
						+ "		ELSE null END AS type_of_visit, last_reason.encounter_datetime FROM (\n"
						+ "		SELECT p.patient_id, MAX(e.encounter_datetime) visit_date FROM patient p\n"
						+ "			INNER JOIN encounter e ON e.patient_id = p.patient_id\n"
						+ "			INNER JOIN obs o ON o.encounter_id = e.encounter_id\n"
						+ "				WHERE p.voided = 0 AND e.voided = 0 AND o.voided = 0 AND e.encounter_type = 21 \n"
						+ "					AND o.concept_id = 1981\n"
						+ "					AND e.location_id = :location AND e.encounter_datetime BETWEEN ( :startDate - INTERVAL 15 DAY) AND ( :endDate - INTERVAL 15 DAY)\n"
						+ "						GROUP BY p.patient_id\n"
						+ "	)last_visit\n"
						+ "	INNER JOIN (\n"
						...
						+ "				AND o.concept_id = 1981\n"
						+ "					GROUP BY patient_id\n"
						+ "	)last_reason ON last_visit.patient_id = last_reason.patient_id AND visit_date = encounter_datetime\n"
						+ ")type_of_visit ON type_of_visit.patient_id = p.patient_id\n"
						+ "\n"
						+ "LEFT JOIN\n"
						+ "(\n"
						+ "	SELECT last_visit.patient_id, last_volunteer.encounter_datetime, last_volunteer.value_text as volunteer FROM (\n"
						+ "		SELECT p.patient_id, MAX(e.encounter_datetime) visit_date FROM patient p\n"
						+ "			INNER JOIN encounter e ON e.patient_id = p.patient_id\n"
						+ "			INNER JOIN obs o ON o.encounter_id = e.encounter_id\n"
						+ "				WHERE p.voided = 0 AND e.voided = 0 AND o.voided = 0 AND e.encounter_type = 21 \n"
						...
						+ "	)last_visit\n"
						+ "	INNER JOIN (\n"
						+ "		SELECT o.person_id patient_id, o.value_text, e.encounter_datetime FROM obs o\n"
						+ "			INNER JOIN encounter e ON e.encounter_id = o.encounter_id\n"
						+ "				WHERE e.voided = 0 AND o.voided = 0 AND e.encounter_type = 21\n"
						+ "				AND e.location_id = :location AND e.encounter_datetime BETWEEN ( :startDate - INTERVAL 15 DAY) AND ( :endDate - INTERVAL 15 DAY)\n"
						+ "				AND o.concept_id = 1912\n"
						+ "					GROUP BY patient_id\n"
						+ "	)last_volunteer ON last_visit.patient_id = last_volunteer.patient_id AND visit_date = encounter_datetime\n"
						+ ")volunteer ON volunteer.patient_id = p.patient_id\n"
						+ "LEFT JOIN\n"
						+ "(\n"
