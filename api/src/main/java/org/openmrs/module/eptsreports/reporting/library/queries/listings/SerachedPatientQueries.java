...
public interface SerachedPatientQueries {
  ...
  class QUERY {
    public static final String findSearchedPatientsList =
        "SELECT p.patient_id, pi.identifier, CONCAT(pn.given_name, ' ', COALESCE(pn.middle_name, ''), ' ', pn.family_name)name, pe.gender, (YEAR( :endDate) - YEAR(pe.birthdate)) age, patient_contact, pa.address1, pa.address3, pa.address5, sector, type_of_visit, start_art.art_start_date, expected_date, GREATEST(DATEDIFF( :endDate, expected_date), 0) missing_days, patients_found.last_visit, last_encounter.encounter_date, volunteer, agreed_date FROM patient p\n"
